...
type EventTypeLabel = "All" | "Politics" | "Economics" | "Business" | "Other";
type EventTypeValue = "" | "politics" | "economics" | "business" | "other";
...
export default function EventsFilters({
  ...
  kinds: string[];
  selectedKind: string;
  eventTypes: Array<{ label: EventTypeLabel; value: EventTypeValue }>;
  selectedType: EventTypeLabel;
  sort: SortMode;
  date?: string;
  q?: string;
}) {
  ...
  function hrefWith({
    ...
    const kindVal = kind ?? selectedKind;
    if (kindVal && kindVal !== "All") params.set("kind", kindVal);

    const typeVal = type ?? (selectedType === "Politics"
      ? "politics"
  ...
  }, [query, q, router, sort, selectedKind, selectedType, date]);
  ...
  return (
    <div className="grid gap-2 md:gap-3">
      ...
      {(date || q || (selectedKind && selectedKind !== "All") || (selectedType && selectedType !== "All")) && (
        <div className="text-xs text-zinc-600 dark:text-zinc-400">
          ...
          {(date || q) && (selectedKind !== "All" || selectedType !== "All") ? (
            <span className="mx-1 xs:mx-2">•</span>
          ) : null}
          {selectedType !== "All" ? <span>Type: {selectedType}.</span> : null}
          {selectedType !== "All" && selectedKind !== "All" ? <span className="mx-1 xs:mx-2">•</span> : null}
          {selectedKind !== "All" ? <span>Kind: {selectedKind}.</span> : null}
        </div>
      )}
    </div>
  );
